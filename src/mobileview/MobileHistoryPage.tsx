...
export interface MobileHistoryOrder {
  id: string;
  table: string;
  item: string;
  count: string;
...
export const MobileHistoryPage: React.FC<MobileHistoryPageProps> = ({
  orders: liveOrders,
  loading: _loading,
  ...
  const displayOrders: MobileHistoryOrder[] = useMemo(() => {
      ...
      <div className="mx-auto min-h-screen w-full max-w-md bg-[#faf9f7] dark:bg-[#16161d] overflow-x-hidden flex flex-col justify-between">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto px-2 sm:px-2.5 pb-24">
          ...
          {/* =================================================
              SEARCH + FILTER
          ================================================= */}
          <section className="mt-2.5 flex gap-1.5">
            <div className="flex h-[40px] flex-1 items-center gap-2 rounded-xl border border-[#e4e7ec] dark:border-zinc-800 bg-white dark:bg-[#1f1f28] px-3 shadow-2xs focus-within:border-[#ff4d0a] transition">
              <Search size={16} className="shrink-0 text-[#101828] dark:text-zinc-400" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search order ID, table, item..."
                className="min-w-0 flex-1 bg-transparent text-[11.5px] text-[#101828] dark:text-white outline-none placeholder:text-[#98a2b3] dark:placeholder:text-zinc-500"
              />
            </div>
            ...
            <button
              ...
              className={`flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-xl border shadow-2xs active:scale-95 transition cursor-pointer ${
                showFilterPills
                  ? "bg-[#fff0e8] dark:bg-[#ff5a1f]/20 text-[#ff4d0a] dark:text-[#ff5a1f] border-[#ff4d0a]"
                  : "bg-white dark:bg-[#1f1f28] text-[#101828] dark:text-zinc-300 border-[#e4e7ec] dark:border-zinc-800"
              }`}
              aria-label="Filter"
            >
          ...
          <div className="mt-2 flex gap-1.5 overflow-x-auto pb-0.5 no-scrollbar">
            {(isSelfPosBilling ? tabs.filter(t => t !== "Cancelled") : tabs).map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`h-7 sm:h-8 shrink-0 rounded-lg border px-3 text-[11px] font-semibold transition-all active:scale-95 cursor-pointer whitespace-nowrap ${
                    active
                      ? "border-[#ff4d0a] bg-[#ff4d0a] text-white shadow-xs"
                      : "border-[#e4e7ec] dark:border-zinc-800 bg-white dark:bg-[#1f1f28] text-[#344054] dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-[#2a2a35]"
                  }`}
                >
                  {tab}
                </button>
          ...
          <section className="mt-2 flex gap-1.5">
            <button
              ...
              className="flex h-[40px] flex-1 items-center gap-2 rounded-xl border border-[#e4e7ec] dark:border-zinc-800 bg-white dark:bg-[#1f1f28] px-3 text-left shadow-2xs active:scale-[0.99] transition cursor-pointer hover:border-slate-300 dark:hover:border-zinc-700"
            >
              <CalendarDays size={16} className="text-[#101828] dark:text-zinc-400 shrink-0" />
              <span className="flex-1 text-[11px] font-medium text-[#344054] dark:text-zinc-300 truncate">
                {dateRangeText}
              </span>
              <ChevronDown size={15} className="text-[#344054] dark:text-zinc-400 shrink-0" />
            </button>

            <button
              type="button"
              onClick={onOpenCalendar}
              className="flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-xl border border-[#e4e7ec] dark:border-zinc-800 bg-white dark:bg-[#1f1f28] text-[#344054] dark:text-zinc-400 shadow-2xs active:scale-95 transition cursor-pointer hover:border-slate-300 dark:hover:border-zinc-700"
        ...
        <MobileFooter activeTab="history" />
      </div>
    </div>
  );
};
